 ...
 * 
 * This is the third assignment for my data structors course. I worked
 * alone on this assignment, in which I implemented a huffman compression
 * algorithm with a custom priority queue that compressed and decompressed
 * a file provided to us. That file was the novel War and Peace by Leo Tolstoy.
 * This presented challenges to me as the file was encoded in UTF-8 and we had to
 * capture each character in a string. Due to the nature of the string class
 * issues arise when doing this which had to be accounted for such and some were not
 * ~yet~ accounted for but were out of the assignment requirements. I attempted to
 * make the algorithm more general to handle more utf-8 dense files but the string
 * class posed many issues.
 * 
 * The other books used for testing are on my to read pile that I thought would
 * make good test cases. 
 * 
 * TODO: Fix bottleneck functions: buildbinaries(),  
 * 
 */
